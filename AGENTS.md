...
- The Shade's behaviour lives in the nested `LegacyHelper.ShadeController` partial class (`LegacyHelper.ShadeController.*.cs`). These files cover movement and the leash (`Movement`), state fields and animation data (`Fields`), persistence hooks (`Persistence`), slash/projectile combat (`Slash`), and charm-driven stat mutation (`Charms`).
- The **Shade AI** - the optional driver that plays the Shade when no second player is - is split deliberately. `Shade/Ai/ShadeAiBrain.cs` decides what to do and **touches no Unity object**, so it is covered by `Tests/ShadeAiBrainTests.cs` rather than only by play sessions; `LegacyHelper.ShadeController.Ai.cs` gathers what it sees and applies what it decides, and `…ShadeController.AiCommand.cs` holds the player's targeting reticle. Keep that line intact - if a decision needs a raycast, put the raycast on the driver side and pass the answer in through `ShadeAiSnapshot`.
- The AI adds no movement or combat code of its own: `Shade/Ai/ShadeAiInput.cs` publishes each decision as one frame of the same inputs a second player would give, so `CaptureMovementInput`, `HandleNailAttack`, `HandleFire` and the rest drive it unchanged. A new Shade ability becomes available to the AI by adding it there, not by teaching the brain to call it.
- Systems under `Shade/` provide the persistent data layer for the companion:
  ...
  - `Shade/Ai/` is the AI described above; it is behaviour rather than persistence, and is the one subfolder here that is not part of the data layer.
