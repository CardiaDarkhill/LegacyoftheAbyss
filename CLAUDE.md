...
- **The companion is respawned on every scene change, and its charm loadout is rebuilt from baseline on every charm change *and* every scene change** (`HandleFinishedEnteringScene` -> `SpawnShadeAtPosition`; `BeginScene` -> `SyncActiveSlot` -> `SetActiveSlot`). So `OnApplied` runs on a *fresh* controller at every door, and **nothing held on the controller can tell a real equip from a rebuild** — a "was this just equipped" diff reads true every room. A hook whose effect is one-off (a heal, a timer reset, a spawn) must decide from state that survives the respawn — `ShadeCharmInventory`, `ShadePersistentState` — or from a value that already accounts for it, such as the restored maximum in `ResolveMaxHpFill`. This has shipped a full heal at every door plus two timers that silently restarted there. Stat modifiers and idempotent flag sets are safe because `ResetStatsToBaseline` zeroes them first; `shadeMaxHP` is **not** among them, being derived output owned by `ApplyCharmHealthModifiers`, and resetting it corrupts the "before" reading the same rebuild compares against.
...
`python Tools/SourceAudit.py` runs five offline checks — `reflection`, `dead`, `duplication`, `perframe`, `subscriptions` — and its docstring records the exact Roslyn route (`-p:EnforceCodeStyleInBuild=true`) for unused members. **Prove a new regression test fails without its fix**, then restore: a setter that already early-outs on an unchanged value will make a "we no longer write" test pass for the wrong reason, and a pinning test can assert something the code never had to compute.
...
- **Charm ids are the save format.** `Collected`, `DiscoveredCharmIds` and `EquippedCharmLoadouts` store `ShadeCharmId` as plain integers — there is no `StringEnumConverter` — so the enum is **append-only**. Inserting a member renumbers every charm below it and hands existing saves the wrong charms, with nothing in the diff to show it. `ShadeCharmInventoryTests` pins the whole map; adding a charm costs one line there.
- **`SetActiveSlot` flushes the slot on every scene entry**, so anything it writes is on a per-room path and needs a dirty check. `ShadeSaveSlotRepository` compares against the JSON it last wrote.
...
- **Check the physics API answers the question asked.** `Collider2D.IsTouching` and maskless `Overlap` consult the layer collision matrix; `Collider2D.Distance` is pure geometry. The wrong one returns "no contact" forever for any layer pair that does not interact — which is most of the interesting ones. The companion's body is on **Default** (only its aggro proxy copies Hornet's layer), so any `IsTouching` against `bodyCol` is a layer-matrix question, not a geometry one; `shade-contact-refused` in a bug report names both layers and whether they really overlapped.
...
- **A cache of Unity objects has two different nulls.** `== null` reports a real null *and* a destroyed object, so a plain dictionary hit keeps a dead sprite forever, while testing `cached != null` re-loads a known *absence* on every call. Use `ReferenceEquals(x, null)` for "we looked and there is none" and the bool operator for "still alive". `ShadeCharacterManager` and `ShadeCharmIconLoader` show the split.
